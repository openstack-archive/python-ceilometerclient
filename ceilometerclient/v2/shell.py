...
def common_alarm_arguments(required):
    def _wrapper(func):
        @utils.arg('--name', metavar='<NAME>', required=required,
...
@common_alarm_arguments(required=True)
...
@common_alarm_arguments(required=False)
@utils.arg('--period', type=int, metavar='<PERIOD>',
           help='Length of each period (seconds) to evaluate over')
@utils.arg('--evaluation-periods', type=int, metavar='<COUNT>',
           help='Number of periods to evaluate over')
