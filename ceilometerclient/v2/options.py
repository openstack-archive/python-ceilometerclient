...
OP_LOOKUP_KEYS = '|'.join(OP_LOOKUP.keys())
OP_PREFIX = r'[^!>=<]'
OP_SPLIT_RE = re.compile(r'^(%(prefix)s.+?)(%(keys)s)(%(prefix)s.+)$' %
                         {'prefix': OP_PREFIX, 'keys': OP_LOOKUP_KEYS})
...
def cli_to_array(cli_query):
    ...
    def split_by_op(query):
        ...
        """
        frags = OP_SPLIT_RE.match(query)

        if not frags:
            raise ValueError('invalid query %(query)s' % {'query': query})
