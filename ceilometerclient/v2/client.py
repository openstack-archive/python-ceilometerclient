...
class Client(object):
    ...
    def _get_alarm_client(self, **kwargs):
        """ Get client for alarm manager that let ceilometerclient redirect to
        aodh endpoint if available.
        """
        alarm_auth_plugin = copy.deepcopy(self.auth_plugin)
        try:
            alarm_auth_plugin.redirect_to_aodh_endpoint(kwargs.get('timeout'))
        except exceptions.EndpointNotFound:
            return self.http_client, False
        alarm_client = client.HTTPClient(
            auth_plugin=alarm_auth_plugin,
            region_name=kwargs.get('region_name'),
            endpoint_type=kwargs.get('endpoint_type'),
            original_ip=kwargs.get('original_ip'),
            verify=kwargs.get('verify'),
